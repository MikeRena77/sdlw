...
# vss2h.pl
# --------
# A utility to convert Visual Source Safe projects into SCM/CCC-Harvest
# repository items.
#
...
#    3.2    8/19/2008  MHA  Further updates for the new Harvest/SCM r12
#    3.3    9/18/2008  MHA  Changed script to begin at root level for SCM repository rather than creating folder with VSS Project name
#    4.0    9/19/2008  MHA  Added new $ver variable and parameter in .ini to pass which version (r12 or r7.1) is being used
#    4.1    10/06/2009  MHA  Discovered $sqlloc could be messed up by pointing somewhere besides $logdir once it gets its final value
# --------------------------------------------------------------------
...
#10) New for r12 - added the package variable to <hcrtpath> command
#
...
#  check for version flag
   if (/^-ver /)
   {
      ...
      {
         chomp($ver);
         if (($ver eq "r12") or ($ver eq "r7.1"))
         {
         }
         else
         {
            die "Invalid version choice: either r12 or r7.1\n";
         }
      }
   }
...
#  if the version is still empty after attempting to load the variable from the ini file, then no version was specified
if ($ver eq "")
{
    warn "The version is set at: $ver\n";
    die "You must specify a version - either r12 or r7.1!\n";
}
...
$logdir = $logdir . "\\" . $ver;
mkdir ("$logdir", 0755) or warn "Can't mkdir $logdir: $!\n";

$logdir = $logdir . "\\" . $vss_projects[0];
mkdir ("$logdir", 0755) or warn "Can't mkdir $logdir: $!\n";
...
$ver	          = $ver; # Version of SCM/Harvest (r12 or r7.1)
...
# Added to overcome pointing location for sql scripts somewhere other than where they get generated
$sqlloc="$logdir";
...
foreach $VSS (@vss_projects)
{
   ...
   ITEM: while (<FILELIST>)
   {
      ...
      next ITEM if /^$projbase.dll$/;       # skip all the builds of the binary (there can be thousands)
      next ITEM if /^$projbase.pdb$/;       # skip all the builds of the binary (there can be thousands)
      ...
      if (/^\$\/.*:$/)                       # project directory in form of $/project/subproj:
      {
         ...
         #  This is original - I don't want to propogate the SourceSafe project name into the repository - MHA 9/18/08        
         # $newwdir = $wdir . "/" . $projbase . $subproj;
         $newwdir = $wdir . $subproj;
         ...
         print "New ViewPath is \"$newviewpath\"\n";

         # This change is for the new SCM r12 changes to the hcrtpath command - MHA 9/18/08                  
         if ($ver eq "r12")
         {
			print ("hcrtpath -b \"$broker\" -en \"$env\" -st \"$state\" -rp \"$newviewpath\" -usr \"$husr\" -pw \"$hpw\" -p \"$pkg\" -cipn \"$ciproc\" -oa \"$logdir\\fileNewPath$$.log\" \n");
				system ("hcrtpath -b \"$broker\" -en \"$env\" -st \"$state\" -rp \"$newviewpath\" -usr \"$husr\" -pw \"$hpw\" -p \"$pkg\" -cipn \"$ciproc\" -oa \"$logdir\\fileNewPath$$.log\" ") == 0
					or warn "hcrtpath -b \"$broker\" -en \"$env\" -st \"$state\" -rp \"$newviewpath\" -usr \"$husr\" -pw \"$hpw\" -p \"$pkg\" -cipn \"$ciproc\" -oa \"$logdir\\fileNewPath$$.log\" failed: $? $! \nARE YOU SURE THIS IS R12?\n";
		 }
		 else
		 {
