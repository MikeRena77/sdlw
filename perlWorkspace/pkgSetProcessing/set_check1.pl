...
foreach $pkg (@ARGV)
{
  ...
  if ("$pkgset" eq "-1" )
  {
    push @all_pkgsets, substr($pkg, 0, -2);
  }
  elsif ("$pkgset" eq "-2" )
  {
    push @all_pkgsets, substr($pkg, 0, -2);
...
foreach $pkgset (@pkgsets)
{
   ...
   foreach $pkg (@pkgs)
   {
      ...
      if ("$pkg2" eq "$pkg")
      {
         print "Match on $pkg2 and $pkg\n";
         $detect_pkg2 = 1;
      }
   }
   
   if ($detect_pkg0 && $detect_pkg1 && $detect_pkg2)
   {
      print "Promoting Packages $pkg0, $pkg1, and $pkg2.\n";
      ...
      system "hpp -usr \"$husername\" -pw \"$huserpass\" -en \"$env\" -st \"$st\" -pn \"Promote\" \"$pkg2\" ";
      # do an error check
      system "del hpp.log";
   }
   elsif ($detect_pkg0 && $detect_pkg1)
   {
      print "Package $pkg2 is missing from state $st\n";
      $stat = -2;
   }
   elsif ($detect_pkg2)
   {
      print "Package $pkg0 and $pkg1 are missing from state $st\n";
      $stat = -2;
   }
   else
   {
