...
$ScriptVersion = '$Header: /CVS/openmake64/tomcat/webapps/openmake.ear/openmake.war/scripts/Dot\040Net\040Solution\040Build.sc,v 1.48 2008/05/09 19:06:39 jim Exp $';
...
if($sln ne "")
{
 ...
 {
  @msbuild_projects = ();
  @non_msbuild_projects = unique($RelDeps->getExt(qw(.CSPROJ .VBPROJ .VCPROJ .BTPROJ)));
 }
...
##############################
{
 ...
 foreach $tmpStr ($RelDeps->getExtList(qw(.vcproj)))
 {
...
sub MSBuildProject
{
 ...
 foreach my $proj ( @{$msbuild_projects_list} )
 {
  ...
  if ( $proj =~ m{(_om_\d{8})\.\w{2}proj$} )
  {
   ...
   my $real_name = '';
   eval { $real_name = realpath( $proj); };  #-- need real_path to resolve .. in names
   if ( ! -e $real_name )
   {
   ...
   while ( ( $proj_ref = $dnet->getNextProjectReference() ) ne '' )
   {
    ...
    if ( -e $new_proj )
    {
     ...
     my $real_name = realpath( $proj); #-- need real_path to resolve .. in names
