    ...
    close (ENVLIST);
        ...
        print ("hco \"bin\\Release\\AAFES.HR.Workers._2k8.dll\" -b \"PRODSWA\" -en \"$project\" -st \"$state\" -vp \"\\$viewpath\" -p \"$package\" -up -replace all -ced -op pc -cp \"$clientpath\" -pn \"Engineering Build CheckOut\" -eh \"%HARVESTHOME%\\hsync.dfo\" -oa \">>&STDOUT\" -wts \n");
        $error_exit=system("hco \"bin\\Release\\AAFES.HR.Workers._2k8.dll\" -b \"PRODSWA\" -en \"$project\" -st \"$state\" -vp \"\\$viewpath\" -p \"$package\" -up -replace all -ced -op pc -cp \"$clientpath\" -pn \"Engineering Build CheckOut\" -eh \"%HARVESTHOME%\\hsync.dfo\" -oa \">>&STDOUT\" -wts");
