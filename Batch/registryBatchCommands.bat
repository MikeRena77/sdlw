rem Here are several registry queries piped to a find command
reg query HKLM\Software /s /f "*.ocx" |find /n /i "target"
reg query HKLM\Software /s /f "*.ocx" |find /n /i "wayne"
reg query HKLM\Software /s /f "*.ocx" |find /n /i "Chevron"

rem The reg query can also be run against remote machines if you have permissions to access the machine
reg query \\tnwp011355.genpitfi01.og.ge.com\HKLM\Software /s /f "*.ocx"
reg query \\tnwp011355.genpitfi01.og.ge.com\HKLM\Software /s /f "Wayne"
reg query \\tnwp011355.genpitfi01.og.ge.com\HKLM\Software /s /f "*.ocx"



exit
rem everything past this exit is strictly information for the reg query command
reg query /?
